...
events.onceKeyDown("KEYCODE_VOLUME_UP", function (event) {
...
if (id("tv_item_label").textMatches(/连签\d+天/).exists()) return true;
...
toastLog("checkin");
var check_in = text("早起打卡");

if (!check_in.findOnce().parent().click()) return false;
...
if (btn = desc("打卡").exists()) {
    toastLog("已打卡");
    success2 = true;
    
} else {

  if (btn = text("打卡").findOne(this.options.timeout)) {
